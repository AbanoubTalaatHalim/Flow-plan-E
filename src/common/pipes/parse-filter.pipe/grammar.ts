...
export const grammar: Grammar = {
  ...
  ParserRules: [
    ...
    {
      name: 'comparison',
      symbols: ['prop', '_', { literal: '=' }, '_', 'string_like'],
      postprocess: comparison('like'),
    },
    {
      name: 'comparison',
      symbols: ['prop', '_', { literal: '=' }, '_', 'string_range'],
      postprocess: comparison('range'),
    },
    ...
    {
      name: 'number',
      symbols: ['number$ebnf$2', { literal: '.' }, 'number$ebnf$3'],
      postprocess: (d, c) => literal('Number', parseFloat(join(d)), c),
    },
    {
      name: 'string_contains',
      symbols: [{ literal: '*' }, 'native_string', { literal: '*' }],
      postprocess: (d, c) => literal('String', d[1], c, 'contains'),
    },
    {
      name: 'string_like',
      symbols: [{ literal: '*' }, 'native_string'],
      postprocess: (d, c) => literal('String', d[1], c, 'endsWith'),
    },
    {
      name: 'string_like',
      symbols: ['native_string', { literal: '*' }],
      postprocess: (d, c) => literal('String', d[0], c, 'startsWith'),
    },
    { name: 'string_like', symbols: ['string_contains'], postprocess: id },
    {
      name: 'range_group',
      symbols: [{ literal: '(' }, '_', 'string_range', '_', { literal: ')' }],
      postprocess: (d, c) => d[2],
    },
    {
      name: 'string_range$string$1',
      symbols: [{ literal: '.' }, { literal: '.' }, { literal: '.' }],
      postprocess: (d) => d.join(''),
    },
    {
      name: 'string_range',
      symbols: ['string', '_', 'string_range$string$1', '_', 'string'],
      postprocess: (d, c) => literal('String', [d[0].value, d[4].value], c, 'range'),
    },
    {
      name: 'string_range$string$2',
      symbols: [{ literal: '.' }, { literal: '.' }, { literal: '.' }],
      postprocess: (d) => d.join(''),
    },
    {
      name: 'string_range',
      symbols: ['number', '_', 'string_range$string$2', '_', 'number'],
      postprocess: (d, c) => literal('Number', [d[0].value, d[4].value], c, 'range'),
    },
